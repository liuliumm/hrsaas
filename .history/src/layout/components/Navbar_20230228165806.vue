...
export default {
  ...
  methods: {
    ...
    async logout() {
      // 这里与登录页面不同写法时因为登陆页面使用了辅助函数mapActions进行了映射    user模块下的logout
      await this.$store.dispatch('user/logout') //// 这里不论写不写 await 登出方法都是同步的（因为removeToken，removeUserInfo没有发起网络请求）
      // this.$router.push('/login')
      this.$router.push(this.$route.query.redirect || '/login')       //跳转到登录页面
      // 上述代码使用query，是因为query的作用--从 URL 的 search 部分提取的已解码查询参数的字典--，后面跟了重定向
      // 还可以使用如下代码
