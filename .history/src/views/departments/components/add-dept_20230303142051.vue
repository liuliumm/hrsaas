...
export default {
  ...
  data() {
    ...
    const checkNameRepeat = async (rule, value, callback) => {    //静态作用域：在定义的位置  自内向外找  注意：不管它在哪里使用。
      const { depts } = await getDepartments().catch(err => err)     // 动态作用域： this  根据不同的运行条件和环境来决定this的值是谁，
      if (!Array.isArray(depts)) {  //判断一下获取到的数据是不是数组         // 这里使用动态作用域虽然也可以，但是这个查找很繁琐，所以我们使用const来声明一个变量，从而指定我们所需要的this指向
        depts = []
      }
      const isRepeat = depts
        .filter(item => item.pid === this.treeNode.id)   // 该方法会在找到的时候停止遍历，而不是将整个数据遍历完
        .some(item => item.name === value);           // 判断是否有相同的name
    ...
    return {
      formData: {  // pid在此处不需要声明为响应式数据，因为并没有用到，相反，如果声明了而不用，反而会浪费性能，因为这会导致该数据被做一次数据拦截。
        name: "", //部门名称
        code: "", //部门编码
        manager: "", //部门管理者
        introduce: "", //部门介绍
      },
      formDataRules: {  // triggers的值'blur',是默认值，此处可省略不写，required只写一次就行了
        name: [
        ...
        manager: [
          { trigger: "blur", message: "部门负责人不能为空" },
          {
            message: "部门负责人",
          },
        ],
  ...
  methods: {
    ...
    // 点击确定时触发
    btnOK() {  //这里的校验是对表单整体的校验，所以使用ref获取组件实例时，要获取整体的form表单
      this.$refs.deptForm.validate(async (isOK) => {
        if (isOK) {  //或在此处直接取isok的反值（!isOk），然后return跳出条件
        try {
          await addDepartments({ ...this.formData, pid: this.treeNode.id });
          this.$emit("AddDept");  //触发父组件监听的事件
