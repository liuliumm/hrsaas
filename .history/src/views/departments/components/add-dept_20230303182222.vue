<template>
  ...
  <!-- 新增部门的弹层 -->
  <el-dialog :title="this.formData.id ? '编辑部门' : '新增部门'" :visible="showDialog" @close="btnCancel">
    <!-- //visible：控制是否显示 -->
...
<script>
import { getDepartments, addDepartments, getDepartmentsById } from "@/api/departments";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      formDataRules: {
        // triggers的值'blur',是默认值，此处可省略不写，required只写一次就行了
        name: [
  ...
  methods: {
    async getDepartmentsById(){
      this.formData = await getDepartmentsById()
      this.$emit('editDept')
    },
    btnCancel() {
    ...
    async getEmployeeSimple() {  // 新增部门时的负责人一栏的数据列表
      // const peoples = []
