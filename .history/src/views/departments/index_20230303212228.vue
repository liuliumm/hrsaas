...
export default {
  ...
  methods: {
    ...
    async editDept(node) {
      this.node = node;
      // await this.$refs.addDeptForm.getDepartmentsById(node.id);

      // 方法1 通过ref找到add-dept组件实例，更新该实例的formData数据
      // this.$refs.addDept.formData.id = node.id
      // this.$refs.addDept.formData = node // 直接把node赋值给formData表单数据即可
      // 这里 没有使用法1,因为该方法获取到的数据有更大可能是旧数据
