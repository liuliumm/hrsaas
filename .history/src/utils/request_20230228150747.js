...
service.interceptors.request.use(// 请求拦截器
    config => {  //config 就是某一次具体请求的具体信息：url baseURL data params header
    // 拦截改造config
        // 在这个位置需要统一的去注入token
...
service.interceptors.response.use(// 响应拦截器
    response => {
        ...
        const { success, message, data, code} = response.data  //code指当前状态，http的状态码为三位，token过期时返回状态码为200，接口内部数据状态码（称为业务状态码）为10002
                                                                    // code  一般为五位以上。
                                                                    // data  对应的结果
                                                                    // message  文字提示
    ...
    //     console.log(err);
        // console.log(Object.keys(err)) // err是一个对象，所以打印出对象的属性
        // console.log(err.?); //根据上面打印出的对象属性来确定错误    Object.keys() 方法会返回一个由一个给定对象的自身可枚举属性组成的数组，数组中属性名的排列顺序和正常循环遍历该对象时返回的顺序一致。
